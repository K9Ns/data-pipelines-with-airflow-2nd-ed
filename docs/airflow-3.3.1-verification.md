...
**3.3.1 비호환으로 확인된 예제 코드는 없습니다.** 임포트 실패는 전부 장 전용 의존성이나 실행 환경 요건이었고, 충족 후 재검사에서 해소되었습니다. 독립적인 2차 정적 검토에서도 제거 API·템플릿 변수·구성 키의 비호환은 0건이었고, 버전과 무관한 콜백 시그니처 문제 1건(발견 6)만 새로 확인되었습니다.
